...
Note that tests in this repo aim to highlight that security is part of your domain and non-functional requirements. Thus we should identify requirements, test cases and add tests to prove that our defence layers work as expected, according to the simple access model requirement "All Product API endpoints require that a client shall only access products for a given market". 

In this repo we have added tests in two projects, one for unit tests with all dependencies mocked and the other for complete system tests where no dependencies are mocked. Larger projects would most likely require more structure and tests on different levels of integration. The important part is what kind of tests we added to verify step 2-6 in our model for secure APIs, this is not a complete test suite for a real-world product API.
...
Note that in a real world system this would often involve a database or external service, and then we would need integration tests as well.
...
Note that the unit tests for the Product service also verify the correct audit logging in order to assert requirements for accountability.
...
Note that for system tests the token service (at https://localhost:4000) and the API (at https://localhost:5000) need to be started.

Start with: `dotnet run --urls https://*:4000` and `dotnet run --urls https://*:5000`

For localhost, the .NET development certificate needs to be trusted, run
